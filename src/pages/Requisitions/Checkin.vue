    ...
    <div class="q-mb-xl">
      <q-list bordered class="rounded-borders">
        ...
        <q-expansion-item
          ...
          :caption="`Costo Total $ ${bucketCostReceived}`"
          dark
  ...
  computed: {
    ...
    originProducts() {
      if (this.order) {
        return this.order.products.map(p => {
          ...
          p.ordered.amount = p.ordered.toReceived ? p.ordered.amount : 0;
    ...
    filterAvailable() {
      switch (this.selectAvailable.value) {
        case 2:
          return this.listProducts.filter(stock => !stock.ordered.toReceived);
        case 3:
    ...
    inBucket() {
      return this.filterAvailable.filter(prod => !prod.ordered.toReceived);
    },
    outBucket() {
      return this.filterAvailable.filter(prod => prod.ordered.toReceived);
    },
