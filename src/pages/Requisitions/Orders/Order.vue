  ...
  <q-page>
    <q-header elevated class="bg-darkl1 text-grey-5">
      <q-toolbar>
        <q-btn
          ...
          flat
        />
        <span class="q-pl-md">Pedido {{ ordercatch.id }}</span>
        <q-space />
    ...
    <div class="row q-pt-sm q-pl-sm">
      <div
        ...
        v-for="(prod, idxprod) in products"
        :key="idxprod"
      >
        <q-card class="bg-darkl1 q-mb-sm q-mr-sm" @click="selItem(prod)">
          ...
          <q-banner
            ...
            v-if="prod.ordered.comments"
            >{{ prod.ordered.comments }}</q-banner
          >
        </q-card>
      </div>
    </div>
    ...
    <q-footer class="bg-darkl1 text-white" elevated v-if="order">
      <div class="q-pa-xs row justify-between items-center">
                ...
                <template v-slot:no-option>
                  <q-item
                    ><q-item-section class="text-grey"
                      >Sin coincidencias</q-item-section
                    ></q-item
                  >
                </template>
                  ...
                  >
                    <div class="text-bold">
                      ...
                      <q-chip
                        ...
                        text-color="white"
                        icon="warning"
                        :label="scope.opt.status.name"
                      />
                    </div>
                    <div caption class="text--2">
        ...
        <template v-else-if="order.status.id == 5"
...
export default {
  ...
  async beforeMount() {
    ...
    this.saveNameExport = `FOLIO-${this.ordercatch.id}`;
    ...
    this.$sktRestock.on("order_changestate", (data) => {
      this.sktOrderHere(data) ? this.sktOrder_changeState(data) : null;
    });
  },
  beforeDestroy() {
  ...
  methods: {
    sktOrderHere(data) {
      return this.ordercatch.id == data.order.id;
    },
    ...
    removeProduct() {
      ...
      let data = {
        ...
        _requisition: this.ordercatch.id,
      };
      ...
      dbreqs
        ...
        .then((success) => {
          ...
          this.$sktRestock.emit("order_update", {
            user: this.profile,
            from: this.workin,
            cmd: "remove",
            order: this.ordercatch,
            product: proderase,
          });
        })
        .catch((fail) => {
